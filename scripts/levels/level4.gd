...
func _ready():
	...
	var level_up = get_tree().get_root().find_node("cell4",true,false)
	level_up.connect("level_up4",self,"level4")
...
func level4():
	anim.play("fade_in")
	yield(get_tree().create_timer(0.5), "timeout")
	get_tree().change_scene("res://scenes/level_ups/Levelup4.tscn")
