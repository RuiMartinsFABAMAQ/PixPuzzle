...
func _ready():
	...
	var level_up = get_tree().get_root().find_node("cell5",true,false)
	level_up.connect("level_up5",self,"level5")
...
func level5():
	anim.play("fade_in")
	yield(get_tree().create_timer(0.5), "timeout")
	get_tree().change_scene("res://scenes/level_ups/Levelup5.tscn")
